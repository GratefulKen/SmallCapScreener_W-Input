...
# --- Sidebar User Input ---
st.sidebar.header("Enter Stock Tickers")
user_input = st.sidebar.text_input(
    "Enter one or more stock tickers separated by commas (e.g., AAPL, MSFT, NVDA):",
    value="BJRI, BLMN, CORT, BOOT"
)
...
if tickers:
    ...
    if not filtered_df.empty:
        selected = st.selectbox("Select a stock to view details", filtered_df["Ticker"].tolist())
        if selected:
